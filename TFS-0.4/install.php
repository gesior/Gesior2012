...
{
	echo('In file <b>install.php</b> must be your IP!<br />In file is:<b>' . $installIP . '</b><br />Your IP is:<b>' . $_SERVER['REMOTE_ADDR'] . '</b>');
	exit;
}
...
elseif($page == 'step')
{
	...
	if($step == 'start')
	{
		...
		$writeable = array('config/config.php', 'cache', 'cache/DONT_EDIT_usercounter.txt', 'cache/DONT_EDIT_serverstatus.txt', 'custom_scripts', 'install.txt');
	...
	elseif($step == 3)
	{
		...
		$tables = array();
		$tables['z_ots_comunication'] = "CREATE TABLE `z_ots_comunication` (
		...
		$tables['z_shop_offer'] = "CREATE TABLE `z_shop_offer` (
							  `id` int(11) NOT NULL auto_increment,
		...
		$tables['z_shop_history_item'] = "CREATE TABLE `z_shop_history_item` (
							  `id` int(11) NOT NULL auto_increment,
		...
		$tables['z_forum'] = "CREATE TABLE `z_forum` (
							  `id` int(11) NOT NULL auto_increment,
		...
		$tables['guild_wars'] = "CREATE TABLE `guild_wars` (
		...
		$tables['guild_wars_table_references'] = "ALTER TABLE `guild_wars`
								  ADD CONSTRAINT `guild_wars_ibfk_1` FOREIGN KEY (`guild_id`) REFERENCES `guilds` (`id`) ON DELETE CASCADE,
								  ADD CONSTRAINT `guild_wars_ibfk_2` FOREIGN KEY (`enemy_id`) REFERENCES `guilds` (`id`) ON DELETE CASCADE;";
		$tables['guild_kills'] = "CREATE TABLE `guild_kills` (
							  `id` INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
							  `guild_id` INT NOT NULL,
							  `war_id` INT NOT NULL,
							  `death_id` INT NOT NULL
							) ENGINE = InnoDB;";
		$tables['guild_kills_table_references'] = "ALTER TABLE `guild_kills`
		...
		// tables code is for MySQL only, sorry
