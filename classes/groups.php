...
class Groups implements Iterator, Countable
{
	...
	public function __construct($file)
	{
		$XML = new DOMDocument();
		if(!$XML->load($file))
			throw new RuntimeException('Groups::__construct - cannot load file <b>' . htmlspecialchars($file) . '</b>');
		...
		foreach($XML->getElementsByTagName('group') as $group)
		{
			if($group->hasAttribute('id') && $group->hasAttribute('name'))
			{
				...
				exit('Cannot load group. <b>id</b> or/and <b>name</b> parameter is missing');
		}
	}
	/*
	 * Get group
