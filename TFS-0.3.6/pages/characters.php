...
if(!empty($name))
{
	...
	if($player->isLoaded())
	{
		...
		$main_content .= '<tr bgcolor="' . $bgcolor . '"><td width="20%">Name:</td><td style="font-weight:bold;color:' . (($player->isOnline()) ? 'green' : 'red') . '">' . htmlspecialchars($player->getName()) . ' ' . $skull . ' <img src="./images/flags/' . $account->getFlag() . '.png" title="Country: ' . $account->getFlag() . '" alt="' . $account->getFlag() . '" />';
