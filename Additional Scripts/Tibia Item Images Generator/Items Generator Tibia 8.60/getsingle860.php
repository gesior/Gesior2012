...
$empty = sha1(file_get_contents('empty_860.gif'));
...
/* READ DAT */
$dat = fopen('./data860/Tibia.dat', 'rb');
...
for($i = 100; $i <= $item_id_in_dat_file; $i++) // it's binary file, we must read all until we go to ID that we need
{
	...
	$sprites = unpack('S*', fread($dat, 2 * $sprites_count)); // in 9.60 it's stored in 'long' (32 bit, not old 'short' - 16 bit), so we read 4 bytes for each
}
fclose( $dat );
/* CLOSE DAT */
...
{
	copy('sprites_unpacked_860/' . $sprite . '.gif', 'items_parts/' . $item_ots_id . '_' . $i . '.gif');
}
